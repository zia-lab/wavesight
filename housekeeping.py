...
def house_keeper(waveguide_id):
    '''
    This function puts in order the files generated by a given
    simulation. It moves the err, out, and fig files to their
    respective folders. It also deletes the bash scripts from
    the code directory.

    If it runs without problems it sends a notification of completion.
