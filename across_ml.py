...
def meta_duct(waveguide_id, mode_idx):
    ...
    expanded_params_fname = [fname for fname in job_dir_contents if fname.endswith('jsonc')][0]
    ...
    printer("getting final coordinates")
    coords = {}
    ...
    with h5py.File(h5_fname,'w') as h5_file:
        h5_file.create_dataset('on_axis_eps', data = on_axis_eps)
        for key, val in coords.items():
            h5_file.create_dataset(key, data = val)
        if save_cross_eps:
            h5_file.create_dataset('cross_eps_at_ml', data = cross_eps)
